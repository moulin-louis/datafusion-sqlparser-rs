        ...
        match chars.peek() {
            Some(&ch) => match ch {
                ...
                '0'..='9' | '.' => {
                    ...
                    // `tuple.1`, treat a dot that directly follows a complete expression as
                    // a field access operator rather than as the start of a decimal number,
                    // so the digits that follow are tokenized as a separate number.
                    //
                    // The preceding token is what tells the two apart: only a token that can
                    // end an expression can be followed by field access. `)` and `]` cover
                    // `tuple(1, 2).1` and `arr[1].2`, which is where positional access is
                    // most commonly used.
                    //

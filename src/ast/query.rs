...
/// first and the name second (see [`ScalarCte`]), and permits both forms to be
...
impl fmt::Display for CteTable {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            CteTable::Cte(cte) => cte.fmt(f),
            CteTable::Scalar(scalar) => scalar.fmt(f),
        }
    }
}
...
impl fmt::Display for ScalarCte {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
